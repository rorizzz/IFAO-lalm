...
def _peek_model_type(model_name_or_path: str) -> str:
    """Read ``model_type`` from config.json without going through AutoConfig.

    AutoConfig dispatches via a registry that only knows model types built into
    transformers, which fails for custom types like ``qwen3_asr`` that live in
    third-party packages. Local checkpoints always carry a config.json, so
    peeking the raw JSON lets us route without triggering that dispatch.
    """
    import json
    ...
    if cfg_path.is_file():
        return json.loads(cfg_path.read_text()).get("model_type", "")
    # Remote (HF hub id) or non-standard layout — fall back to AutoConfig.
    return getattr(AutoConfig.from_pretrained(model_name_or_path), "model_type", "")
...
def _load_encoder(
    ...
    if model_type in {"qwen3_asr", "qwen3_asr_audio_encoder"}:
        ...
        if model_type == "qwen3_asr":
            ...
            encoder = Qwen3ASRAudioEncoder.from_pretrained(
                model_name_or_path, torch_dtype=torch_dtype
            )
        return encoder, int(encoder.config.output_dim)
    ...
    raise ValueError(
        ...
        "'qwen3_omni_moe_audio_encoder', 'qwen3_asr', "
